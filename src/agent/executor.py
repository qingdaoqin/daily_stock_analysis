...
### 1. 入场纪律与交易约束（按市场区别处理）
- A股：存在涨跌停、T+1 和做空限制，政策本身常是主因；乖离率 < 2% 为最佳买点，2-5% 可小仓介入，> 5% 默认观望
- 港股：不存在 A股式涨跌停限制，更关注公告、南向资金、行业景气与市场风险偏好；可用公告确认位、回测位和均线支撑入场
- 美股：不存在 A股式涨跌停/T+1 约束，更关注市场环境、财报、公司指引、估值和事件驱动；> 5% 只代表追价风险，不等于自动看空或卖出

### 2. 趋势交易（顺势而为）
- A股：MA5 > MA10 > MA20 仍是买入级信号的重要硬条件
- 港股：优先看趋势与公告催化共振，不能只按 A股 MA5/MA10 模板判断
- 美股：均线趋势要和财报、市场环境、行业主线、估值一起判断，不能只因离均线较远就直接否决强趋势股
- 均线发散上行优于均线粘合
...
{{
    ...
    "dashboard": {{
        ...
        "data_perspective": {{
            ...
            "chip_structure": {{"profit_ratio": 0, "avg_cost": 0, "concentration": 0, "chip_health": "健康/一般/警惕/不适用"}}
        }},
        "intelligence": {{
            "latest_news": "",
            "risk_alerts": [],
...
### 买入（60-79分）：
- ✅ 趋势结构成立或催化明确
- ✅ A股/港股乖离率 <5%；美股若财报和市场共振，可放宽但必须写清“回踩更优，不宜追价”
- ✅ 量能正常
- ⚪ 允许一项次要条件不满足

### 观望（40-59分）：
- ⚠️ A股/港股乖离率过高属于强警示；美股高乖离率只代表追价风险
- ⚠️ 均线缠绕趋势不明
- ⚠️ 有风险事件
