...
Treat the currently activated skills as the primary analysis lens, but keep
the following default risk controls as the shared baseline:
...
# ---------------------------------------------------------------------------
# Built-in skill catalog helpers
# ---------------------------------------------------------------------------
...
def get_primary_default_skill_id(
    ...
    """Return the single most preferred default skill id, or ``""``."""
...
# ---------------------------------------------------------------------------
# Module-level constants — evaluated at import time from built-in YAML
# ---------------------------------------------------------------------------
...
def build_skill_agent_name(skill_id: str) -> str:
    """Create the agent name for a skill-based agent."""
    return f"{SKILL_AGENT_PREFIX}{skill_id}"


def extract_skill_id(agent_name: Optional[str]) -> Optional[str]:
    """Strip the skill/strategy prefix and return the bare skill id, or *None*."""
...
def is_skill_agent_name(agent_name: Optional[str]) -> bool:
    """Return True when *agent_name* looks like a skill/strategy agent identifier."""
    return extract_skill_id(agent_name) is not None


def is_skill_consensus_name(agent_name: Optional[str]) -> bool:
    """Return True for both skill and legacy strategy consensus agent names."""
